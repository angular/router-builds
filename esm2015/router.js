...
function containsParams(a, b) {
    const /** @type {?} */ names = Object.keys(b);
    if (names.length > Object.keys(a).length)
        return false;
    return names.every((name) => {
        if (!a.hasOwnProperty(name))
            return false;
        const /** @type {?} */ aValue = a[name];
        const /** @type {?} */ aIsArray = Array.isArray(aValue);
        const /** @type {?} */ bValue = b[name];
        const /** @type {?} */ bIsArray = Array.isArray(bValue);
        if (aIsArray !== bIsArray)
            return false;
        return aIsArray ?
            aValue.length === bValue.length &&
                (/** @type {?} */ (aValue)).every(v => (/** @type {?} */ (bValue)).indexOf(v) > -1) :
            aValue === bValue;
    });
}
...
class UrlSegment {
     ...
     */
    toString() {
        const /** @type {?} */ path = encode(this.path);
        const /** @type {?} */ strParams = paramsToStrings(this.parameters).join(';');
...
function serializeQueryParams(params) {
    const /** @type {?} */ strParams = paramsToStrings(params).join(`&`);
...
function inheritedParamsDataResolve(route) {
    ...
    let /** @type {?} */ inheritingStartingFrom = pathToRoot.length - 1;
    while (inheritingStartingFrom >= 1) {
        const /** @type {?} */ current = pathToRoot[inheritingStartingFrom];
        const /** @type {?} */ parent = pathToRoot[inheritingStartingFrom - 1];
...
function tree(oldSegmentGroup, newSegmentGroup, urlTree, queryParams, fragment) {
    let /** @type {?} */ qp = convertToParams(queryParams);
...
class Router {
    ...
    createUrlTree(commands, navigationExtras = {}) {
        ...
        const /** @type {?} */ convertedQueryParams = convertToParams(/** @type {?} */ ((queryParams)));
        let /** @type {?} */ q = null;
        if (queryParamsHandling) {
            switch (queryParamsHandling) {
                case 'merge':
                    q = Object.assign({}, this.currentUrlTree.queryParams, convertedQueryParams);
                    break;
                case 'preserve':
    ...
    navigateByUrl(url, extras = { skipLocationChange: false }) {
        const /** @type {?} */ urlTree = typeof url === 'string' ? this.parseUrl(url) : url;
     ...
     */
    isActive(url, exact) {
        const /** @type {?} */ tree = typeof url === 'string' ? this.urlSerializer.parse(url) : url;
